  ...
  describe 'Assessments' do
    ...
    it 'can be created' do
      response = api_instance.create_assessment(
        member_version_stable_id: 'TEST-Test-AND-TEST-MORE',
        content: { text: Faker::Lorem.paragraph }.to_json,
        variants: 2.times.map{|question_index|
          {
            format_id: format.id,
            member_variant_id: question_index.to_s,
            content: {
      ...
      expect(assessment.member_version_stable_id).to eq('TEST-Test-AND-TEST-MORE')
